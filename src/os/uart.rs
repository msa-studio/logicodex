...
macro_rules! uart_print {
    ($($arg:tt)*) => ({
        use core::fmt::Write;
        let mut writer = $crate::os::uart::UartWriter;
...
mod tests {
    ...
    #[test]
    fn uart_ports_are_correct() {
        assert_eq!(PORT_DATA, 0x3F8);
        assert_eq!(PORT_LSR, 0x3FD);
        assert_eq!(PORT_LCR, 0x3FB);
        assert_eq!(PORT_DLL, 0x3F8);
        assert_eq!(PORT_DLH, 0x3F9);
    }

    #[test]
    fn vga_buffer_address() {
        assert_eq!(VGA_BUFFER, 0xB8000 as *mut u8);
    }
    ...
    #[test]
    fn vga_color_attribute() {
        let attr = vga_entry_color(VgaColor::White, VgaColor::Black);
        assert_eq!(attr, 0x0F, "White on Black = 0x0F");

        let attr = vga_entry_color(VgaColor::Yellow, VgaColor::Blue);
        assert_eq!(attr, 0x1E, "Yellow on Blue = 0x1E");
    }
    ...
    fn vga_writer_new() {
        ...
        assert_eq!(writer.color, 0x0F);
    }

    #[test]
    fn vga_writer_write_string() {
        let mut writer = VgaWriter::new();
        writer.write_string("Hello");
        assert_eq!(writer.col, 5);
        assert_eq!(writer.row, 0);
    }

    #[test]
    fn vga_writer_new_line() {
        let mut writer = VgaWriter::new();
    ...
    fn uart_hex_format() {
        ...
        // (can't test output in hosted environment)
        uart_hex(0xDEADBEEF);
        uart_hex(0);
        uart_hex(u64::MAX);
    }
    ...
    #[test]
    fn uart_decimal_format() {
        uart_decimal(0);
        uart_decimal(42);
        uart_decimal(1234567890);
        uart_decimal(u64::MAX);
    }
}
