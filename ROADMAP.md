...
- [X] **Issue #01 — Formal EBNF Grammar Definition (COMPLETED / SOLVED — EXECUTABLE IN v1.21-alpha):** Document and implement the exact grammar rules for both **Novice Pseudocode** and **Expert Shorthand** to eliminate parsing ambiguity.
- [X] **Issue #02 — Undefined Behavior Catalog & Pointer Provenance (COMPLETED / SOLVED — EXECUTABLE IN v1.21-alpha):** Define and implement explicit rules governing raw pointer operations, physical memory-mapped boundaries, hosted-memory isolation, freestanding memory-access constraints, and zero-overhead severity mitigation tiers.
...
|---|---|---|---|
| Issue #01 | [X] COMPLETED / SOLVED — EXECUTABLE IN v1.21-alpha | Mohamad Supardi Abdul | 1. Formal 4-layer grammar checked in as a living document inside `spec/v1.21-alpha/UpdateIssue1-ebnf.md`.<br>2. Lexer and recursive-descent parser implement canonical novice/expert tokens, typed bindings, functions, returns, hardware declarations, and provenance-aware address literals.<br>3. Logical validation passes through `scripts/validate_v121_executable_logic.py`. |
| Issue #02 | [X] COMPLETED / SOLVED — EXECUTABLE IN v1.21-alpha | Mohamad Supardi Abdul | 1. UB and pointer-provenance catalog checked in at `spec/v1.21-alpha/UpdateIssue2-provenance.md`.<br>2. Semantic analysis implements static division-by-zero, bounds, resource, target-policy, hosted/freestanding provenance, and severity diagnostics.<br>3. Code generation accepts the expanded AST and emits severity-aware safe executable paths where applicable. |
